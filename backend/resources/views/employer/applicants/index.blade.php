...
<div class="view">
  <div style="display:flex;gap:9px;flex-wrap:wrap;margin-bottom:18px">
    @foreach (['' => 'الكل', 'submitted' => 'جديد', 'accepted' => 'مقبول', 'rejected' => 'مرفوض'] as $key => $label)
      @php($on = ($status ?? '') === $key)
      <a href="{{ route('employer.applicants.index', array_filter(['status' => $key, 'q' => $search ?? ''])) }}" class="btn" style="border:1px solid {{ $on ? '#2B2724' : '#E7E3DA' }};background:{{ $on ? '#2B2724' : '#fff' }};color:{{ $on ? '#fff' : '#6C665C' }};font-size:14px;font-weight:800;padding:9px 18px;border-radius:12px">{{ $label }}</a>
    @endforeach
  </div>
  ...
  @else
    <div style="background:#fff;border:1px solid #EDEAE2;border-radius:20px;overflow:hidden">
        ...
        <div class="rowh" style="display:flex;align-items:center;gap:16px;padding:18px 22px;border-bottom:1px solid #F2F0E9">
          ...
          <div style="flex:1;min-width:0">
            <div style="display:flex;align-items:center;gap:9px">
              ...
              <a href="{{ route('employer.applicants.show', $application->id) }}" style="font-size:16.5px;font-weight:800;color:#2B2724">{{ $name }}</a>
            ...
            <div style="font-size:13.5px;color:#8A857A;font-weight:600;margin-top:3px">تقدّم على {{ $application->job->title }}@if ($profile?->city) · {{ $profile->city->name }}@endif @if ($profile?->years_of_experience) · {{ $profile->years_of_experience }} سنوات @endif · طلب #{{ $application->reference_number }}</div>
          </div>
          ...
          @elseif ($application->conversation)
            ...
            <a href="{{ route('employer.messages.show', $application->conversation->uuid) }}" class="btn" style="display:flex;align-items:center;gap:7px;height:42px;padding:0 18px;border-radius:12px;background:#F5F3EC;border:1px solid #E7E3DA;color:#2B2724;font-size:14px;font-weight:800"><i class="iconsax" style="font-size:18px" icon-name="messages-2"></i>مراسلة</a>
          @endif
        </div>
      @endforeach
    </div>

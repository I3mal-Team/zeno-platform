...
<div class="view">
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px">
    @foreach ($stats as $stat)
      <div class="cardh" style="background:#fff;border:1px solid #E5EAE6;border-radius:20px;padding:20px">
        ...
        <div style="font-size:32px;font-weight:900;color:#22302A;margin-top:16px">{{ $stat['value'] }}</div>
        <div style="font-size:14px;color:#7E8B84;font-weight:700;margin-top:2px">{{ $stat['label'] }}</div>
      </div>
    @endforeach
  </div>
  ...
  <div style="display:grid;grid-template-columns:1.6fr 1fr;gap:16px;margin-top:16px">
    ...
    <div style="background:#fff;border:1px solid #E5EAE6;border-radius:20px;padding:24px">
      <div style="display:flex;align-items:center;justify-content:space-between">
        <div>
          <div style="font-size:18px;font-weight:800;color:#22302A">الطلبات هذا الأسبوع</div>
          <div style="font-size:13.5px;color:#7E8B84;font-weight:600;margin-top:2px">إجمالي {{ $chartTotal }} طلبًا@if ($chartDelta !== null) · {{ $chartDelta >= 0 ? '+' : '' }}{{ $chartDelta }}% عن الأسبوع الماضي @endif</div>
        </div>
        <div style="display:flex;align-items:center;gap:6px;font-size:13px;font-weight:800;color:#8A6D12"><span style="width:10px;height:10px;border-radius:3px;background:#C9A24B"></span>الطلبات</div>
      </div>
      <div style="display:flex;align-items:flex-end;justify-content:space-between;gap:14px;height:190px;margin-top:24px">
        @foreach ($chart as $bar)
          <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:9px;height:100%;justify-content:flex-end">
            <div style="width:100%;max-width:38px;border-radius:9px 9px 4px 4px;background:{{ $bar['highlight'] ? '#C9A24B' : '#EDE6D2' }};height:{{ max($bar['height'], 3) }}%"></div>
            <div style="font-size:12.5px;color:#869089;font-weight:700">{{ $bar['day'] }}</div>
          </div>
        @endforeach
      </div>
    </div>
    ...
    {{-- Quick actions --}}
    <div style="background:#fff;border:1px solid #E5EAE6;border-radius:20px;padding:24px">
      <div style="font-size:18px;font-weight:800;color:#22302A">إجراءات سريعة</div>
      <div style="display:flex;flex-direction:column;gap:11px;margin-top:16px">
        <a href="{{ route('employer.jobs.create') }}" class="btn" style="display:flex;align-items:center;gap:13px;background:#F1EFE4;border:1px solid #F1E6C4;border-radius:14px;padding:14px">
          <div style="width:42px;height:42px;border-radius:12px;background:#C9A24B;display:flex;align-items:center;justify-content:center;color:#284C3D;flex:0 0 auto"><i class="iconsax" style="font-size:21px" icon-name="add"></i></div>
          <div style="flex:1"><div style="font-size:15px;font-weight:800;color:#284C3D">نشر وظيفة جديدة</div><div style="font-size:12.5px;color:#9A857A;font-weight:600">تصل للباحثين القريبين فورًا</div></div>
        ...
        <a href="{{ route('employer.applicants.index') }}" class="btn" style="display:flex;align-items:center;gap:13px;background:#F7F5EF;border:1px solid #EBE7DE;border-radius:14px;padding:14px">
          ...
          <div style="flex:1"><div style="font-size:15px;font-weight:800;color:#284C3D">مراجعة المتقدّمين</div><div style="font-size:12.5px;color:#869089;font-weight:600">{{ $newApplications > 0 ? 'لديك '.$newApplications.' طلبات جديدة' : 'لا توجد طلبات جديدة' }}</div></div>
          <i class="iconsax" style="font-size:19px;color:#BAC4BD" icon-name="chevron-left"></i>
        </a>
        <a href="{{ route('employer.messages.index') }}" class="btn" style="display:flex;align-items:center;gap:13px;background:#F7F5EF;border:1px solid #EBE7DE;border-radius:14px;padding:14px">
          ...
          <div style="flex:1"><div style="font-size:15px;font-weight:800;color:#284C3D">الرسائل</div><div style="font-size:12.5px;color:#869089;font-weight:600">تواصل مع المتقدّمين</div></div>
          <i class="iconsax" style="font-size:19px;color:#BAC4BD" icon-name="chevron-left"></i>
        </a>
      </div>
    </div>
  ...
  {{-- Recent applicants --}}
  <div style="background:#fff;border:1px solid #E5EAE6;border-radius:20px;padding:8px 8px 6px;margin-top:16px">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 18px 12px">
      <div style="font-size:18px;font-weight:800;color:#22302A">أحدث المتقدّمين</div>
    ...
    @forelse ($recent as $application)
      ...
      <a href="{{ route('employer.applicants.show', $application->id) }}" class="rowh" style="display:flex;align-items:center;gap:14px;padding:13px 18px;border-radius:14px">
        ...
        <div style="flex:1;min-width:0">
          <div style="font-size:15.5px;font-weight:800;color:#284C3D">{{ $name }}</div>
          <div style="font-size:13px;color:#7E8B84;font-weight:600;margin-top:2px">{{ $application->job->title }}@if ($profile?->city) · {{ $profile->city->name }}@endif @if ($profile?->years_of_experience) · {{ $profile->years_of_experience }} سنوات خبرة @endif</div>
        </div>
        <div style="font-size:13px;color:#869089;font-weight:700">{{ $application->created_at->diffForHumans() }}</div>
    ...
    @empty
      <div style="padding:34px 18px 40px;text-align:center">
        ...
        <div style="font-size:17px;font-weight:800;color:#22302A;margin-top:14px">لا يوجد متقدمون بعد</div>
        <div style="font-size:14px;color:#7E8B84;font-weight:600;margin-top:5px">انشر إعلاناً لتبدأ باستقبال الطلبات.</div>
      </div>
    @endforelse
  </div>
</div>

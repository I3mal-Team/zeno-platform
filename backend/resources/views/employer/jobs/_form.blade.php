...
<form method="POST" action="{{ $action }}" style="display:flex;flex-direction:column;gap:18px;max-width:760px">
  @csrf
  ...
  <div class="cardh" style="background:#fff;border:1px solid #EDEAE2;border-radius:20px;padding:22px;display:flex;flex-direction:column;gap:16px">
    ...
    <div>
      ...
      <textarea name="description" placeholder="تفاصيل المهام والمتطلبات…" style="width:100%;height:120px;resize:none;border:1px solid {{ $errors->has('description') ? '#B23232' : '#E7E3DA' }};border-radius:14px;background:#FBFAF7;padding:14px 16px;font-family:inherit;font-size:15px;font-weight:600;color:#2B2724;outline:none">{{ old('description', $j?->description) }}</textarea>
    </div>
  </div>

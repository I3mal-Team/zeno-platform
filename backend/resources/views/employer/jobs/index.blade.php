...
<div class="view">
  <div style="display:flex;gap:9px;flex-wrap:wrap;margin-bottom:18px">
    @foreach (['' => 'الكل', 'active' => 'نشطة', 'paused' => 'موقوفة', 'pending_review' => 'قيد المراجعة'] as $key => $label)
      @php($on = ($status ?? '') === $key)
      <a href="{{ route('employer.jobs.index', array_filter(['status' => $key])) }}" class="btn" style="border:1px solid {{ $on ? '#2B2724' : '#E7E3DA' }};background:{{ $on ? '#2B2724' : '#fff' }};color:{{ $on ? '#fff' : '#6C665C' }};font-size:14px;font-weight:800;padding:9px 18px;border-radius:12px">{{ $label }}</a>
    @endforeach
  </div>
  ...
  @else
    <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:16px">
      @foreach ($jobs as $job)
        ...
        <div class="cardh" style="background:#fff;border:1px solid #EDEAE2;border-radius:20px;padding:22px">
          <div style="display:flex;align-items:flex-start;gap:14px">
            ...
            <div style="flex:1;min-width:0">
              <div style="display:flex;align-items:center;gap:9px">
                <a href="{{ route('employer.jobs.edit', $job->uuid) }}" style="font-size:18px;font-weight:800;color:#2B2724">{{ $job->title }}</a>
              ...
              <div style="font-size:13.5px;color:#8A857A;font-weight:600;margin-top:3px">{{ $job->workType->name }}@if ($job->district) · {{ $job->district->name }}@endif @if ($job->published_at) · {{ $job->published_at->diffForHumans() }}@endif</div>
            </div>

            <div style="text-align:left;flex:0 0 auto">
              <div style="font-size:18px;font-weight:900;color:#2B2724">{{ $job->formattedSalary() }}</div>
              <div style="font-size:11.5px;color:#9A958A;font-weight:700">{{ $job->salaryUnit->name }}</div>
            </div>
          </div>
          ...
          <div style="display:flex;align-items:center;gap:10px;margin-top:16px;padding-top:16px;border-top:1px solid #F0EEE7">
            <div style="flex:1;display:flex;align-items:center;gap:8px">
              <div style="width:34px;height:34px;border-radius:10px;background:#F5F3EC;display:flex;align-items:center;justify-content:center;color:#8A857A"><i class="iconsax" style="font-size:17px" icon-name="user-1"></i></div>
              <div style="line-height:1.2"><div style="font-size:15px;font-weight:900;color:#2B2724">{{ $job->live_applications_count ?? 0 }}</div><div style="font-size:11px;color:#9A958A;font-weight:700">متقدّم</div></div>
            </div>
            <div style="flex:1;display:flex;align-items:center;gap:8px">
              <div style="width:34px;height:34px;border-radius:10px;background:#F5F3EC;display:flex;align-items:center;justify-content:center;color:#8A857A"><i class="iconsax" style="font-size:17px" icon-name="eye"></i></div>
              <div style="line-height:1.2"><div style="font-size:15px;font-weight:900;color:#2B2724">{{ $job->views_count }}</div><div style="font-size:11px;color:#9A958A;font-weight:700">مشاهدة</div></div>
            </div>

            <a href="{{ route('employer.jobs.edit', $job->uuid) }}" class="btn" title="تعديل" style="width:38px;height:38px;border-radius:11px;background:#F5F3EC;color:#6C665C;display:flex;align-items:center;justify-content:center;flex:0 0 auto"><i class="iconsax" style="font-size:18px" icon-name="edit-2"></i></a>

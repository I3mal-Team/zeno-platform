  ...
  <div style="background:#fff;border:1px solid #E5EAE6;border-radius:20px;padding:24px">
    <div style="display:flex;align-items:center;gap:14px">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-size:18px;font-weight:800;color:#22302A">{{ $organization->name }}</div>
        <div style="font-size:13.5px;color:#7E8B84;font-weight:600;margin-top:3px">الحالة الحالية: {{ $organization->verification_status->label() }}</div>
      </div>
      <span style="font-size:12.5px;font-weight:800;padding:6px 12px;border-radius:9px;background:{{ $organization->verification_status === VerificationStatus::Verified ? '#E7F4EC' : '#F3ECD6' }};color:{{ $organization->verification_status === VerificationStatus::Verified ? '#1F8A4D' : '#8A6D12' }}">{{ $organization->verification_status->label() }}</span>
    </div>
    ...
    <div style="margin-top:20px;padding-top:20px;border-top:1px solid #F0EEE7">
      @if ($organization->verification_status === VerificationStatus::Verified)
        <div style="font-size:15px;color:#284C3D;font-weight:700;line-height:1.8">منشأتك موثّقة. إعلاناتك تُنشر مباشرة بدون مراجعة، وتظهر بشارة «منشأة موثّقة» للمرشحين.</div>

      @elseif ($request && $request->status === VerificationRequestStatus::Pending)
        <div style="font-size:15px;color:#284C3D;font-weight:700;line-height:1.8">طلبك قيد المراجعة — أُرسل {{ $request->created_at->diffForHumans() }}.</div>
        <div style="font-size:13.5px;color:#7E8B84;font-weight:600;margin-top:6px">أرفقت {{ $request->getMedia(App\Models\VerificationRequest::DOCUMENTS_COLLECTION)->count() }} مستند. سنبلغك بالنتيجة.</div>
      ...
      @else
        @if ($request && $request->status === VerificationRequestStatus::Rejected)
          <div style="background:#FBEDEA;border-radius:14px;padding:14px 16px;margin-bottom:18px">
            ...
            @endif
            <div style="font-size:13px;color:#7E8B84;font-weight:600;margin-top:6px">صحّح المستندات وأعد الإرسال.</div>
          </div>
        @endif

        <div style="font-size:15px;font-weight:800;color:#22302A">أرفق مستندات التوثيق</div>
        <div style="font-size:13.5px;color:#7E8B84;font-weight:600;margin-top:5px;line-height:1.8">
        ...
        <form method="POST" action="{{ route('employer.verification.store') }}" enctype="multipart/form-data" style="margin-top:18px">
          @csrf
          <input type="file" name="documents[]" multiple required accept="image/jpeg,image/png,application/pdf" style="width:100%;border:1.5px dashed #DCE3DD;border-radius:14px;background:#F7F9F7;padding:16px;font-family:inherit;font-size:14px;font-weight:600;color:#284C3D">
          @error('documents')<div style="font-size:13px;color:#B23232;font-weight:700;margin-top:6px">{{ $message }}</div>@enderror
          @error('documents.*')<div style="font-size:13px;color:#B23232;font-weight:700;margin-top:6px">{{ $message }}</div>@enderror

          <button type="submit" class="btn" style="margin-top:16px;display:flex;align-items:center;gap:8px;height:46px;padding:0 22px;border-radius:13px;background:#284C3D;border:none;cursor:pointer;color:#fff;font-family:inherit;font-size:15px;font-weight:800"><i class="iconsax" style="font-size:19px;color:#C9A24B" icon-name="verify"></i>إرسال طلب التوثيق</button>
        </form>
      @endif
    </div>
  </div>

...
<section style="max-width:520px;margin:0 auto;padding:48px 24px 60px">
  <div style="background:#fff;border:1px solid #EFEDE6;border-radius:26px;padding:34px">
    <h1 style="font-size:28px;font-weight:900;color:#211F20">تسجيل الدخول</h1>
    <p style="font-size:15px;color:#6C665C;font-weight:500;margin-top:8px;line-height:1.7">أدخل رقم جوالك وسنرسل لك رمز تحقّق — بدون كلمة مرور.</p>
    ...
    <form method="POST" action="{{ route('login.code') }}" style="margin-top:24px;display:flex;flex-direction:column;gap:18px">
      ...
      <div>
        <div style="font-size:14px;font-weight:800;color:#2B2724;margin-bottom:9px">أدخل بصفتك</div>
      ...
      <div>
        <div style="font-size:14px;font-weight:800;color:#2B2724;margin-bottom:9px">رقم الجوال</div>
        <div style="display:flex;align-items:center;height:52px;border:1px solid {{ $errors->has('phone') || $errors->has('country') ? '#B23232' : '#E7E3DA' }};border-radius:14px;background:#FBFAF7;overflow:hidden">
          @php($selectedIso = old('country', 'SA'))
          @php($selectedCountry = $countries->firstWhere('iso2', $selectedIso) ?? $countries->first())
          <input id="phone-input" name="phone" value="{{ old('phone') }}" inputmode="numeric" placeholder="{{ $selectedCountry?->placeholder ?? '5X XXX XXXX' }}" style="flex:1;height:100%;border:none;background:transparent;padding:0 14px;font-family:inherit;font-size:15px;font-weight:700;color:#2B2724;outline:none;direction:ltr;text-align:left">
          <select id="country-select" name="country" style="height:100%;border:none;border-right:1px solid #EFEDE6;background:#F4F1EA;padding:0 12px;font-family:inherit;font-size:14px;font-weight:800;color:#2B2724;outline:none;cursor:pointer;direction:ltr">

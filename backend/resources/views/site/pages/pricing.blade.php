...
<section style="max-width:1180px;margin:0 auto;padding:64px 26px 30px;text-align:center">
  <div style="display:inline-flex;align-items:center;gap:8px;background:#fff;border:1px solid #E5EAE6;border-radius:100px;padding:7px 15px;font-size:13.5px;font-weight:800;color:#8A6D12">باقات أصحاب العمل</div>
  <h1 style="font-size:46px;font-weight:900;color:#22302A;margin-top:18px;line-height:1.2;letter-spacing:-.01em">أسعار بسيطة<br>تنمو مع منشأتك</h1>
  <p style="font-size:17px;color:#5C6862;font-weight:500;line-height:1.7;margin:16px auto 0;max-width:560px">ابدأ مجانًا، وارتقِ عندما تحتاج مزيدًا من الإعلانات والوصول. بلا عقود ولا رسوم خفية.</p>
  ...
  <div style="display:inline-flex;align-items:center;gap:4px;background:#F1EEE7;border-radius:100px;padding:5px;margin-top:28px">
    <a href="{{ route('site.pricing') }}" style="padding:9px 22px;border-radius:100px;font-size:14.5px;font-weight:800;background:{{ $yearly ? 'transparent' : '#fff' }};color:{{ $yearly ? '#7E8B84' : '#284C3D' }}">شهري</a>
    <a href="{{ route('site.pricing', ['cycle' => 'year']) }}" style="display:flex;align-items:center;gap:7px;padding:9px 22px;border-radius:100px;font-size:14.5px;font-weight:800;background:{{ $yearly ? '#fff' : 'transparent' }};color:{{ $yearly ? '#284C3D' : '#7E8B84' }}">
...
<section style="max-width:1180px;margin:0 auto;padding:0 26px">
  <div class="g-3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:18px;align-items:start">
    @foreach ($plans as $plan)
      <div style="position:relative;background:{{ $plan['cardBg'] }};border:1px solid {{ $plan['border'] }};border-radius:26px;padding:30px 26px">
        @if ($plan['popular'])
          <span style="position:absolute;top:-13px;right:50%;transform:translateX(50%);background:#C9A24B;color:#284C3D;font-size:12.5px;font-weight:800;padding:5px 14px;border-radius:100px;white-space:nowrap">الأكثر شيوعًا</span>
        @endif
  ...
  <p style="text-align:center;font-size:13.5px;color:#7E8B84;font-weight:600;margin-top:26px">جميع الأسعار بالريال السعودي · شاملة ضريبة القيمة المضافة · يمكنك الإلغاء في أي وقت</p>
</section>
...
<section style="max-width:980px;margin:0 auto;padding:64px 26px 20px">
  ...
  <div style="background:#fff;border:1px solid #E5EAE6;border-radius:22px;margin-top:36px;overflow-x:auto">
    <table style="width:100%;border-collapse:collapse;min-width:560px">
      <thead>
        <tr style="background:#F7F9F7">
          @foreach (['الميزة', 'مجاني', 'أعمال', 'مؤسسات'] as $index => $header)
            <th style="padding:16px 20px;text-align:right;font-size:14.5px;font-weight:800;color:#284C3D{{ $index === 2 ? ';background:#F1EFE4' : '' }}">{{ $header }}</th>
          @endforeach
        </tr>
      </thead>
      <tbody>
        @foreach ($comparison as $row)
          <tr style="border-top:1px solid #F2EFE8">
            ...
            <td style="padding:15px 20px;font-size:14.5px;font-weight:700;color:#5C6862">{{ $row['free'] }}</td>
            <td style="padding:15px 20px;font-size:14.5px;font-weight:800;color:#284C3D;background:#F1EFE4">{{ $row['business'] }}</td>
            <td style="padding:15px 20px;font-size:14.5px;font-weight:700;color:#5C6862">{{ $row['enterprise'] }}</td>
          </tr>
        @endforeach
      </tbody>
    </table>
...
<section style="max-width:1180px;margin:0 auto;padding:64px 26px 20px">
  <div style="background:linear-gradient(155deg,#315C4A,#223F31);border-radius:30px;padding:52px 44px;text-align:center">
    <h2 style="font-size:34px;font-weight:900;color:#fff;letter-spacing:-.01em">ابدأ التوظيف اليوم — مجانًا</h2>
    <p style="font-size:17px;color:#B8C4BC;font-weight:500;margin-top:12px">أنشئ حساب منشأتك وانشر أول وظيفة خلال دقائق.</p>
    <div style="display:flex;gap:12px;justify-content:center;margin-top:26px;flex-wrap:wrap">
      <a href="{{ route('employer.dashboard') }}" class="btn" style="background:#C9A24B;color:#284C3D;font-size:15px;font-weight:800;padding:14px 26px;border-radius:15px">افتح لوحة التحكم</a>
